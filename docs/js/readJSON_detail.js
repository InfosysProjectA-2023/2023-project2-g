		...
		const json_url = "https://infosysprojecta-2023.github.io/2023-project2-g/data.json";
		...
		const fetch_json = async (url) => {//urlの文字列のURLからidまたはqで指定した値を持つJSONオブジェクトを取得
			...
			if (response.ok) {
				...
				if(main.length > 0){ //<main>～</main>があったら
					...
					let access_1_text = document.getElementById("access_1");
                    if(access_1_text != null){//<p id="access_1"></p>のタグがあったら
                        access_1_text.innerHTML = obj.access_1; //detail（DBではカラム）の値に内容のHTMLを変更
                    }
